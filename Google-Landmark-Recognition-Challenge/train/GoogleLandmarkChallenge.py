...
path_to_images = "train_frequent_50/*"
...
for subdir in sorted(glob.glob(path_to_images)):
	...
	for image in sorted(glob.glob(subdir + "/*")):
		...
		data_dict["Labels"].append(get_index_from_value(most_frequent_50, label))
...
# pickle_idx = open("dataset_indices_50.pickle", "wb")
# pickle.dump(dataset_indices, pickle_idx)
# pickle_idx.close()


pickle_idx = open("dataset_indices_50.pickle", "rb")
...
trainloader = DataLoader(trainset, batch_size = 48, shuffle = True, drop_last = True)
validloader = DataLoader(validset, batch_size = 48, shuffle = True, drop_last = True)
...
name = "resnet18_pretrained_no_augmentation_"
...
holdoutloader = DataLoader(holdoutset, batch_size = 48, shuffle = True, drop_last = True)
